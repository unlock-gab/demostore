...
import {
  Star, CheckCircle, Shield, Truck, Zap, Award,
...
const reviews = [
  { name: "أحمد.م", wilaya: "الجزائر", review: "منتج ممتاز، وصل بسرعة والجودة أحسن من المتوقع! نتائج واضحة خلال أسبوع.", stars: 5 },
  { name: "سارة.ب", wilaya: "وهران", review: "سعر رائع وخدمة ممتازة. أوصي به لكل الجزائريين. الدفع عند الاستلام ريّح بالي.", stars: 5 },
  { name: "يوسف.ك", wilaya: "قسنطينة", review: "تجربة شراء سهلة والمنتج وصل في وقت قياسي. جودة عالية جداً.", stars: 5 },
  { name: "فاطمة.ح", wilaya: "بجاية", review: "ما كنتش متوقعة تنائج بهذه السرعة! شكراً للمتجر على الخدمة الممتازة.", stars: 5 },
  { name: "محمد.ز", wilaya: "سطيف", review: "المنتج حقيقي ومؤثر. وصل في ظرف يومين. سأطلب مرة ثانية بالتأكيد.", stars: 4 },
];
      ...
      <div className="max-w-lg mx-auto px-4 pb-6 space-y-6 pt-5">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="grid grid-cols-3 gap-3">
          {[
            ...
            { icon: ThumbsUp, value: "4.9★", label: "تقييم متوسط" },
          ].map((stat, i) => (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-5">
            ...
            {displayedReviews.map((r, i) => (
              <div key={i} className="bg-white/10 rounded-xl p-3.5">
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
                    {r.name[0]}
                  </div>
          ...
          {reviews.length > 3 && (
            <button
              ...
              className="w-full mt-3 py-2.5 text-fuchsia-300 hover:text-fuchsia-200 text-sm font-medium border border-fuchsia-500/30 hover:border-fuchsia-500/50 rounded-xl transition-all"
            >
              {showAllReviews ? "عرض أقل" : `عرض ${reviews.length - 3} تقييمات أخرى`}
            </button>
          )}
        </motion.div>
        ...
        <p className="text-center text-white/30 text-xs pb-24">
          © {new Date().getFullYear()} Demo Store — هواتف وإكسسوارات
        </p>

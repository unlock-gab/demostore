...
import { Search, Eye, X, Package, Truck, CheckCircle, Clock, XCircle, Phone, MapPin, Home, Building2, UserCheck, ChevronDown, Edit2, Save, RotateCcw, StickyNote, Trash2, AlertTriangle, Send, Hash, ArrowRight } from "lucide-react";
...
const statusConfig: Record<string, { label: string; color: string; icon: any; bg: string }> = {
  ...
  confirmed:   { label: "Confirmée",    color: "text-blue-400",    bg: "bg-blue-500/10 border-blue-500/20",     icon: UserCheck },
  dispatched:  { label: "En dispatch",  color: "text-indigo-400",  bg: "bg-indigo-500/10 border-indigo-500/20", icon: Package },
  in_delivery: { label: "En livraison", color: "text-violet-400",  bg: "bg-violet-500/10 border-violet-500/20", icon: Truck },
  delivered:   { label: "Livrée",       color: "text-emerald-400", bg: "bg-emerald-500/10 border-emerald-500/20", icon: CheckCircle },
  returned:    { label: "En retour",    color: "text-orange-400",  bg: "bg-orange-500/10 border-orange-500/20", icon: RotateCcw },
  cancelled:   { label: "Annulée",      color: "text-red-400",     bg: "bg-red-500/10 border-red-500/20",       icon: XCircle },
};

const statusOptions = ["pending", "confirmed", "dispatched", "in_delivery", "delivered", "returned", "cancelled"];
...
export default function AdminOrders() {
  ...
  const urlStatus = new URLSearchParams(searchQuery).get("status") || "all";
  ...
  const { data: deliveryCompanies = [] } = useQuery<DeliveryCompany[]>({ queryKey: ["/api/delivery-companies"] });
  const enabledCompanies = deliveryCompanies.filter((c: DeliveryCompany) => c.enabled);
  ...
  const createShipmentMutation = useMutation({
    mutationFn: async ({ orderId, companyId }: { orderId: string; companyId: string }) => {
      const res = await apiRequest("POST", `/api/orders/${orderId}/create-shipment`, { companyId });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "فشل إنشاء الشحنة");
      return data;
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["/api/orders"] });
      ...
      if (selectedOrder) {
        setSelectedOrder({ ...selectedOrder, status: "shipped", trackingNumber: data.trackingNumber, shippingCompany: data.company } as any);
      }
    },
    onError: (e: any) => toast({ title: "خطأ في إنشاء الشحنة", description: e.message, variant: "destructive" }),
  });
                            ...
                            {nextStatusMap[order.status] && (() => {
                              ...
                              return (
                                <button
                                  ...
                                  title={`→ ${nextCfg.label}`}
                                  className={`flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-bold border transition-all disabled:opacity-50 ${nextCfg.bg} ${nextCfg.color} hover:opacity-80`}
                            ...
                            {deleteConfirmId === order.id ? (
                              <div className="flex items-center gap-1">
                                <button
                                  ...
                                  className="p-2 bg-red-600 hover:bg-red-700 border border-red-500 text-white rounded-xl transition-all disabled:opacity-60"
                                  data-testid={`button-confirm-delete-${order.id}`}
                                >
                                  {deleteOrderMutation.isPending ? <span className="w-4 h-4 block border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <CheckCircle className="w-4 h-4" />}
                                </button>
                                <button
                                  onClick={() => setDeleteConfirmId(null)}
                                  className="p-2 bg-gray-800 hover:bg-gray-700 border border-gray-700 text-gray-400 rounded-xl transition-all"
                ...
                <div className="p-6">
                  ...
                  <div className="bg-gray-800 rounded-2xl p-4 mb-4 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {(selectedOrder as any).selectedSize && (
                        <div className="text-gray-400 text-xs mt-0.5">المقاس: <span className="text-white font-semibold">{(selectedOrder as any).selectedSize}</span></div>
                      )}
                      {(selectedOrder as any).selectedColor && (
                        <div className="text-gray-400 text-xs mt-0.5">اللون: <span className="text-white font-semibold">{(selectedOrder as any).selectedColor}</span></div>
                      )}
                    ...
                    <>
                      <div className="grid grid-cols-2 gap-3 mb-5">
                        {[
                          ...
                          ...(selectedOrder.commune ? [{ label: "البلدية", value: selectedOrder.commune, icon: "🏘️" }] : []),
                  ...
                  {!editMode && (selectedOrder as any).trackingNumber && (
                    <div className="mb-5 p-4 bg-violet-500/10 border border-violet-500/20 rounded-2xl">
                      <div className="flex items-center gap-2 mb-1">
                        <Hash className="w-4 h-4 text-violet-400" />
                        <span className="text-violet-400 text-sm font-bold">رقم التتبع</span>
                      </div>
                      <p className="text-white font-mono font-bold text-sm">{(selectedOrder as any).trackingNumber}</p>
                      {(selectedOrder as any).shippingCompany && (
                        <p className="text-gray-400 text-xs mt-1">عبر {(selectedOrder as any).shippingCompany}</p>
                      )}
                    </div>
                  )}
                  ...
                  {!editMode && !(selectedOrder as any).trackingNumber && (
                    <div className="mb-5">
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 py-2.5 bg-violet-600/20 border border-violet-500/30 text-violet-400 rounded-xl text-sm font-bold hover:bg-violet-600/30 transition-all"
                            ...
                            <div className="mt-3 space-y-3">
                              {enabledCompanies.length === 0 ? (
                                <p className="text-gray-500 text-xs text-center py-3">
                                  لا توجد شركات توصيل مفعّلة. فعّل شركة من صفحة شركات التوصيل.
                                </p>
                              ) : (
                                <>
                                  <select
                                    ...
                                    className="w-full bg-gray-900 border border-gray-600 rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-violet-500"
                                    data-testid="select-shipping-company"
                                  >
                                    <option value="">-- اختر شركة التوصيل --</option>
                                    {enabledCompanies.map((c: DeliveryCompany) => (
                                      <option key={c.id} value={c.id}>{c.name}</option>
                                    ))}
                                  </select>
                                  <button
                                    onClick={() => selectedCompanyId && createShipmentMutation.mutate({ orderId: selectedOrder.id, companyId: selectedCompanyId })}
                                    disabled={!selectedCompanyId || createShipmentMutation.isPending}
                                    className="w-full py-2.5 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-xl text-sm font-bold hover:from-violet-500 hover:to-purple-500 transition-all disabled:opacity-50"
                                    data-testid="button-confirm-shipment"
                                  >
                                    {createShipmentMutation.isPending ? "جاري الإنشاء..." : "تأكيد إنشاء الشحنة"}
                                  </button>
                                </>
                              )}
                            </div>

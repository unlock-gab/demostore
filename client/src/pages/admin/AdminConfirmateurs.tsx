...
type ConfirmateurStats = { total: number; pending: number; confirmed: number; dispatched: number; in_delivery: number; delivered: number; returned: number; cancelled: number };
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            confirmateurs.map((c, i) => {
              const s = c.stats ?? { total: 0, pending: 0, confirmed: 0, dispatched: 0, in_delivery: 0, delivered: 0, returned: 0, cancelled: 0 };
              const deliveryRate = s.total > 0 ? Math.round((s.delivered / s.total) * 100) : 0;
              const confirmRate = s.total > 0 ? Math.round(((s.confirmed + s.dispatched + s.in_delivery + s.delivered) / s.total) * 100) : 0;
              const retourRate = s.total > 0 ? Math.round(((s.returned + s.cancelled) / s.total) * 100) : 0;
              return (
                <motion.div
                  key={c.id}
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-wrap" data-testid={`stats-confirmateur-${c.id}`}>
                    ...
                    <div className="flex flex-col items-center gap-0.5 px-4 py-2 rounded-xl bg-blue-500/10 border border-blue-500/20 min-w-[64px]">
                      ...
                      <span className="text-base font-black text-blue-400 leading-none">{s.confirmed}</span>
                      <span className="text-[10px] text-blue-500/70 leading-none">Confirmée</span>
                    </div>
                    <div className="flex flex-col items-center gap-0.5 px-4 py-2 rounded-xl bg-indigo-500/10 border border-indigo-500/20 min-w-[64px]">
                      <Package className="w-3.5 h-3.5 text-indigo-400" />
                      <span className="text-base font-black text-indigo-400 leading-none">{s.dispatched}</span>
                      <span className="text-[10px] text-indigo-500/70 leading-none">Dispatch</span>
                    </div>
                    <div className="flex flex-col items-center gap-0.5 px-4 py-2 rounded-xl bg-purple-500/10 border border-purple-500/20 min-w-[64px]">
                      ...
                      <span className="text-base font-black text-purple-400 leading-none">{s.in_delivery}</span>
                      <span className="text-[10px] text-purple-500/70 leading-none">Livraison</span>
                    </div>
                    <div className="flex flex-col items-center gap-0.5 px-4 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 min-w-[64px]">
                      ...
                      <span className="text-[10px] text-emerald-500/70 leading-none">Livrée</span>
                    </div>
                    <div className="flex flex-col items-center gap-0.5 px-4 py-2 rounded-xl bg-orange-500/10 border border-orange-500/20 min-w-[64px]">
                      <XCircle className="w-3.5 h-3.5 text-orange-400" />
                      <span className="text-base font-black text-orange-400 leading-none">{s.returned}</span>
                      <span className="text-[10px] text-orange-500/70 leading-none">Retour</span>
                    </div>
                    <div className="flex flex-col items-center gap-0.5 px-4 py-2 rounded-xl bg-red-500/10 border border-red-500/20 min-w-[64px]">
                      ...
                      <span className="text-[10px] text-red-500/70 leading-none">Annulée</span>
                    </div>
                    {s.total > 0 && (

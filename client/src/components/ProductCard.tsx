...
export default function ProductCard({ product, index = 0 }: ProductCardProps) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -6 }}
      className="group relative bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-md hover:shadow-xl hover:shadow-rose-500/10 transition-all duration-300 cursor-pointer"
      data-testid={`card-product-${product.id}`}
      onClick={() => navigate(`/products/${product.id}`)}
    >
      <div className="relative overflow-hidden bg-gray-50 aspect-square">
        {imgError ? (
          <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-rose-50 to-pink-50 text-rose-300">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-16 h-16 mb-2 opacity-40" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
        ...
        <div className="absolute inset-x-0 bottom-0 p-3 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300">
          <button
            ...
            className="w-full py-2.5 bg-gradient-to-r from-rose-600 to-pink-600 text-white font-bold rounded-xl text-sm shadow-lg hover:opacity-90 transition-all"
      ...
      <div className="p-4">
        ...
        <h3 className="text-sm font-bold text-gray-800 line-clamp-2 leading-tight mb-3 group-hover:text-rose-700 transition-colors">
          {product.name}
        </h3>
        <div className="flex items-center gap-2">
          <span className="text-lg font-bold text-rose-700" data-testid={`text-price-${product.id}`}>

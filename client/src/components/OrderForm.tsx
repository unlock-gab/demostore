...
import { CheckCircle, Loader2, Phone, User, MapPin, Home, Building2, Truck, Package, Ruler, Palette } from "lucide-react";
import { ALGERIAN_WILAYAS, Product, DeliveryPrices, DEFAULT_DELIVERY_PRICES } from "@shared/schema";
import { COMMUNES_BY_WILAYA } from "@shared/communes";
...
export default function OrderForm({ product, source = "product", idPrefix = "" }: OrderFormProps) {
  ...
  const trackAbandonedCart = async (phone: string) => {
    if (cartTracked || phone.length < 9 || orderSuccess) return;
    try {
      const name = form.getValues("customerName");
      const wilaya = form.getValues("wilaya");
      await apiRequest("POST", "/api/abandoned-carts", {
  ...
  const form = useForm<OrderFormValues>({
    ...
    defaultValues: { customerName: "", customerPhone: "", wilaya: "", commune: "", deliveryType: "home", quantity: 1, notes: "", selectedSize: "", selectedColor: "" },
  });
        ...
        <form onSubmit={form.handleSubmit(data => orderMutation.mutate(data))} className="space-y-4">
            ...
            <FormItem>
              ...
              <FormControl>
                <Input
                  ...
                  onBlur={e => { field.onBlur(); trackAbandonedCart(e.target.value); }}
                />
              </FormControl>
              <FormMessage />
            </FormItem>
          ...
          <FormField control={form.control} name="wilaya" render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <select
                  {...field}
                  onChange={e => { field.onChange(e); form.setValue("commune", ""); }}
          ...
          {availableCommunes.length > 0 && (
            <FormField control={form.control} name="commune" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-gray-700 font-semibold flex items-center gap-1.5 text-sm">
                  <Home className="w-4 h-4 text-violet-500" /> البلدية
                </FormLabel>
                <FormControl>
                  <select
                    {...field}
                    className="w-full h-11 px-4 border border-gray-200 rounded-xl text-gray-700 focus:border-emerald-400 outline-none bg-white text-sm"
                    data-testid={`${idPrefix}select-order-commune`}
                  >
          ...
          {hasSizes && (
            <FormField control={form.control} name="selectedSize" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-gray-700 font-semibold flex items-center gap-1.5 text-sm">
                  <Ruler className="w-4 h-4 text-violet-500" /> المقاس
                </FormLabel>
                <FormControl>
          ...
          {hasColors && (
            <FormField control={form.control} name="selectedColor" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-gray-700 font-semibold flex items-center gap-1.5 text-sm">
                  <Palette className="w-4 h-4 text-violet-500" /> اللون
                </FormLabel>
                <FormControl>
                  <div className="flex flex-wrap gap-2">
                    {(product.colors || []).map(color => (
                      <button
                        key={color}
                        type="button"
                        onClick={() => field.onChange(field.value === color ? "" : color)}
                        className={`px-4 py-2 rounded-xl border-2 text-sm font-bold transition-all ${
                          field.value === color
